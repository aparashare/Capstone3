...
from fmm.utils.constants.constants import (WORKSHOP_PICTURE_PATH, ASSIGNMENT_PATH,
...
from fmm.utils.constants.table_names import (WORKSHOP, WORKSHOP_USER, WORKSHOP_COUPON,
                                             WORKSHOP_ASSIGNMENT, WORKSHOP_ASSIGNMENT_USER,
                                             APPOINTMENT, APPOINTMENT_ASSIGNMENT,
                                             APPOINTMENT_ASSIGNMENT_USER, APPOINTMENT_USER,
                                             STUDY_PAYMENT, WORKSHOP_RECORDING, GROUP_CALL)
from fmm.utils.global_request import get_current_user
from fmm.utils.storage import OverwriteStorage, NewFileStorage
from fmm.utils.validators import NotPastValidator, NotFarFutureValidator
...
class AbstractWorkshop(models.Model):
    ...
    picture = models.ImageField(storage=OverwriteStorage(), upload_to=workshop_picture_upload_path)
    ...
    def save(self, *args, **kwargs):
        if self._state.adding and self.picture:
            self.picture.name = (f'{slugify(self.title)}_{slugify(str(timezone.now()))}'
                                 f'.{self.picture.name.split(".")[-1]}')
...
class Workshop(AbstractWorkshop):
    group_call = models.OneToOneField('study.GroupCall', related_name='workshop',
